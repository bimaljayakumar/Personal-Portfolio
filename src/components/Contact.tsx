...
export default function Contact() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const text = `Hi, I'm ${formState.name} (${formState.email})%0A%0A${encodeURIComponent(formState.message)}`;
    window.open(`https://wa.me/919778707336?text=${text}`, "_blank");
    ...
    <section className="relative z-20 bg-[#0a0a0a] py-32 px-4 md:px-12 overflow-hidden" id="contact">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          >
            ...
            <div className="flex flex-col gap-6 mb-12">
              <a href="mailto:bimaljayakumar18@gmail.com" className="flex items-center gap-4 text-gray-300 hover:text-white transition-colors group">
                ...
                <span className="text-lg">bimaljayakumar18@gmail.com</span>
              </a>
          ...
          >
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                type="submit"
                className="w-full py-4 bg-linear-to-r from-blue-600 to-purple-600 rounded-xl text-white font-bold hover:opacity-90 transition-opacity flex items-center justify-center gap-2"
              >
                Send via WhatsApp
              </button>
            </form>
        ...
        <footer className="mt-24 pt-8 border-t border-white/5 text-center text-gray-500 font-mono text-sm">
           <p>&copy; {new Date().getFullYear()} Bimal Jayakumar. All rights reserved.</p>
        </footer>
      </div>
    </section>
  );
